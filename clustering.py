...
def run(args):
    ...
    if args.num_clusters != -1:
        ...
        # KMeans
        # cluster_module = KMeans(n_clusters=args.num_clusters, init='random', random_state=seed, max_iter=1000).fit(X_train)
    elif args.clustering_method == 'MeanShift':
        bandwidth = args.bandwidth if args.bandwidth > 0.0 else None
        cluster_module = MeanShift(bandwidth=bandwidth, max_iter=1000).fit(X_train)
    else:
